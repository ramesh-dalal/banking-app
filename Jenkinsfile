    ...
    stages {
        stage('SCM Checkout') {
            steps {
                ...
                git 'https://github.com/ramesh-dalal/banking-app.git'
                //git 'https://github.com/ramesh-dalal/banking-app.git'
            }
		}
        stage('Maven Build') {
       ...
       stage("Docker build"){
            steps {
				sh 'docker version'
				sh "docker build -t rameshdalal/bankapp-app:${BUILD_NUMBER} ."
				sh 'docker image list'
				sh "docker tag rameshdalal/bankapp-app:${BUILD_NUMBER} rameshdalal/bankapp-app:latest"
            }
        }
		...
		stage('Push2DockerHub') {
			...
			steps {
				sh "docker push rameshdalal/bankapp-app:latest"
			}
		}
